...
## 十一、v1.1.0 迭代记录（2026-05-31）

### 变更1：本地论文去重（Local Cache Check）

**问题**: 用户重复请求同一篇论文时，AI会重新下载、重新分析，浪费时间和token。

**解决方案**:
- 在 **Step 1（识别）** 和 **Step 2（下载）** 之间插入本地缓存检查
- 检查逻辑：
  1. 将论文标题清洗为目录名（与 Step 2 的清洗规则一致）
  2. 检查 `papers/{sanitized_title}/analysis_report/summary.md` 是否存在
  3. 如果用户提供 arXiv ID，同时检查 `papers/paper_{arxiv_id}/analysis_report/summary.md`
  4. 如果存在：直接读取 summary.md 呈现给用户，跳过下载、摘要生成、Git提交，直接进入讨论环节
  5. 如果不存在：正常执行后续流程

**关键设计点**:
- 去重检查发生在"用户确认论文"之后，避免误拦截（比如同名不同论文的情况，先让用户确认）
- 目录名必须严格一致，因此清洗规则必须与 Step 2 完全相同
- 对于 ID 查找，由于现有目录多以标题命名，ID 查找作为兜底策略

**修改文件**:
- `AGENTS.md`: Step 1 新增 "Local Cache Check" 段落，Step 2 开头增加执行前提说明
- `CLAUDE.md`: Step 1 同步更新

---

### 变更2：讨论日志记录（Discussion Logging）

**问题**:
1. 用户与 Agent 的问答历史无法持久化，重启后丢失上下文
2. AI Agent 的聊天窗口不支持 LaTeX 公式渲染，导致数学公式的展示效果极差

**解决方案**:
- 在 **Step 5（后续讨论）** 中增加强制要求：每次问答必须追加到 `analysis_report/discussion_log.md`
- 文件格式：
  ```markdown
  ## 讨论记录 - YYYY-MM-DD
  ...
  **回答：**
  ...
  ```
- 如果文件不存在则自动创建
- 公式使用 Markdown 数学块（`$$...$$` 或 ````math`）确保正确渲染

**关键设计点**:
- 使用单文件持续追加（而非每次新建文件），降低管理复杂度，便于全文搜索历史记录
- 时间戳精确到分钟，足以区分一次会话中的多个问题
- 该文件也应纳入 Git 管理，成为论文分析资料的一部分

**修改文件**:
- `AGENTS.md`: Step 5 新增 "Discussion Logging" 段落
- `CLAUDE.md`: Step 5 同步更新
...
| 文件 | 旧版本 | 新版本 |
|------|--------|--------|
| `AGENTS.md` | 1.0.0 | 1.1.0 |
| `CLAUDE.md` | 1.0.0 | 1.1.0 |
| `DEVELOPMENT_NOTES.md` | 1.0.0 | 1.1.0（新增本章） |
